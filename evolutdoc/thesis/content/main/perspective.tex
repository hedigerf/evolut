...
\chapter{Diskussion und Ausblick\label{chap:perspective}}
  ...
  \section{Diskussion der Resultate\label{sec:diskRes}}
    ...
    \subsection{Wie kann die Geometrie der Tiere evolviert werden?}
      Die Konstruktion der geometrischen Körper aus deren sich ein Individuum zusammensetzt, wurde unter \vref{sub:Beine} und \vref{subsub:GenotypeBodypointCreation} besprochen.
    ...
    \subsection{Individuum Allgemeine Lösung vs Individuum evolvieren auf Evolvierbarkeit}
  ...
  \section{Ausblick\label{sec:ausblick}}
    ...
    \subsection{Hyptothese Allgemeine Lösung vs Evolvierung auf Evolvierbarkeit\label{sub:hypoAnsatz}}
      Leider war nicht genügend Zeit vorhanden folgende Hypothese zu validieren:
      Individuen welche durch die allgemeine Lösungen gefunden werden,
      sollten bei unterschiedlichen Parcours im Durchschnitt besser abschneiden als ihre Gegenspieler~(Evolvierung auf Evolvierbarkeit),
      da sie sich schneller an eine neue Umgebung anpassen können.
      Wenn immer der gleiche Parcour benutzt wird, sollten die Individuen welche auf Evolvierbarkeit evolviert worden sind, auf Dauer überlegen sein. \\
      Um diese Hyptothese zu validieren, sollten zwei Simulationsläufe mit zwei Populationen aus den Lösungsansätzen durchgeführt werden:
      \begin{itemize}
        \item Simulationslauf mit gleichem Parcours
        \item Simulationslauf mit 10 unterschiedlichen Parcours
      \end{itemize}
      Anschliessend sollte ein Vergleich angestellt werden unter den Lösungsansätzen mit beiden Läufen.
