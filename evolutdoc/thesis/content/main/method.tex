...
\chapter{Methode}
  ...
  \section{Design der Individuen}
    ...
    \subsection{Phänotyp\label{sub:Phenotyp}}
      ...
      Aus den reallen Werten des Genotyps wird der Phänotyp in der Physik-Engine erstellt.
      Dieser Prozess wird auch als abbilden des Genotyps auf einen Phäntotyp bezeichnet (\vref{sub:introPhenotyp}).
      Er setzt sich aus mehreren geometrischen Figuren und Constraints~\cite{gamedev:constraints} zusammen, welche die Physik-Engine anbietet.
      ...
      Ein Constraint hat die Funktion 2 Objekte in einer Physik-Engine einzuschränken~\cite{gamedev:constraints}.
  ...
  \section{Motor\label{sec:Engine}}
    ...
    Der Motor bildet zusammen mit den Informationen zum Bewegungsablauf im Phänotyp die Bewegung eines Individuums.
    Im Genotyp ist verankert, wann und welche Bewegung vom Motor ausgeführt wird.
    ...
    \\
    Mit diesen Informationen ist es dem Motor möglich, die Bewegung vollständig zu kontrollieren.
    Der
    \\
    Die Individuen der initialen Population~\vref{sec:InitialPolulation} besitzen eine vorgegebene Bewegung.
    Diese ist der Bewegung von Ameisen nachempfunden. Die Art und Weise,
    wie sich Ameisen fortbewegen ist für \Glspl{Hexapod} eine

    % vorgabe ameise
    % zentrales element mit ablauf liste
    % hexapod -> 2 tripod
