...
\chapter{Methode}
  ...
  \subsection{Motor\label{sub:Engine}}
    ...
    Der Bewegungsmotor kontrolliert die Bewegung eines Phenotyps indem die Motoren der Hüft- und
    Knie-Gelenke kontrolliert werden.

    \subsubsection{Bewegungen\label{subsub:EngineMovements}}

      Dem stehen verschiedene Kategorien von Bewegungen zur Verfügung.
      Es wird unterschieden zwischen:

      \begin{itemize}
        \item Einfache~\ref{EngineMovementsSimple}
        \item Zusammengesetzte~\ref{EngineMovementsCompound}
        \item Prädikate~\ref{EngineMovementsPredicates}
      \end{itemize}

      \paragraph{Einfache Bewegung\label{EngineMovementsSimple}}

        Einfache Bewegungen werden dadurch charakterisiert,
        dass sie einen Parameter einer Extremität des Phenotyps verändern.
        \\
        Verschiedene Arten stehen dabei zur Verfügung:
        \begin{itemize}
          \item Winkelgeschwindigkeit setzten
          \item Winkel setzten
          \item Winkel sperren
        \end{itemize}

        \subparagraph{Winkelgeschwindigkeit setzten\label{subpar:EngineMovementsSimpleSTS}}

          In dieser Bewegung setzt der Motor die Winkelgeschwindigkeit eines Gelenk-Motors.
          Dabei wird die Invariante für die maximale Winkelgeschwindigkeit berücksichtigt.
          \\
          \[\omega_{Max} = 1\frac{rad}{s}\]
      ...
      \paragraph{Zusammengesetzte Bewegung\label{EngineMovementsCompound}}
        ...
        Zusammengesetzte Bewegungen gruppieren eine beliebige Anzahl von Bewegungen.
        Dabei spielt es keine Rolle, von welchem Typ die Bewegung ist.
        ...
        \\
        Als Auswertungsfunktionen stehen zwei Typen zur Verfügung:
        \begin{itemize}
          \item Alle \\
            Alle Teil-Bewegungen in dieser Gruppe müssen erfolgreich ausgeführt worden sein.
          \item Eine \\
            Nur eine einzige Bewegung in dieser Gruppe muss erfolgreich ausgeführt worden sein.
        \end{itemize}
