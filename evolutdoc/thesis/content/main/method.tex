...
\chapter{Methode}
  ...
  \section{Motor\label{sec:Engine}}
    ...
    Der Motor sorgt zusammen mit den Informationen zum Bewegungsablauf im Phänotyp für Bewegung eines Individuums.
    Im Genotyp ist verankert, wann eine Bewegung vom Motor ausgeführt wird.
    ...
    \subsection{Funktionsprinzip}
      ...
      Der Motor überwacht die Übergänge zwischen einzelnen Bewegugnen oder zusammengefassten Bewegungen. % ref
      Nach jedem Übergang zu einer neuen Bewegung, führt der Motor diese aus und prüft,
      ob der Zustand gewechselt werden muss.
      Ein Zustandswechsel ist gleichzusetzten mit eine neue Bewegung auszuführen.
      \\
      ...
      \\
      Das Funktionsprinzip des Motors ist ein \acrfull{fsm}.
      Dabei repräsentieren die (zusammengefassten) Bewegungen die Zustände des Motors.

    \subsection{Vorgegebene Bewegung}

      Die Individuen der initialen Population~\vref{sec:InitialPolulation} besitzen eine vorgegebene Bewegung.
      Der Grund dafür ist, dass damit die Wahrscheinlichkeit,
      dass die Evolution lauffähige Individuen hervorbring steigt.
      \\
      Die vorgegebene Bewegung ist den Ameisen nachempfunden, aufgrund der Einfachheit und Stabilität. 
      Eine Ameise bewegt pro Schritt immer 3 Beine.
      Alternierend wird pro Seite 1 oder 2 Beine verwendet und nach vorne bewegt.
      Diese Bewegung erlaubt es der Ameise stets stabil zu stehen, da ein Dreibein immer stabil steht.
      \\
  ...
  \section{Auswahl des Evolutionären Algorithmus}
    ...
    % TODO konkretisieren und plaudern entfernen

    Das Austauschen der Genen (Rekombination) ist wenig hilfreich bei der Lösung der Problemstellung.
    Man stelle sich dazu vor, dass das Gen welches den Körper definiert, mit dem des Motors vertauscht wird. % form
    ...
    \\
    Eine Baumrepräsentation oder binäre Repräsentation erscheint hier wenig hilfreich. % form
    Ein Punkt ist nichts anderes als zwei reale Werte, % form
    ...
    Die Genetik ist in diesem Falle also vernachlässigbar.
    Schlussendlich wurde entschieden die Problemstellung, auf Grund der gelieferten Argumente,
    mit evolutionärer Programmierung zu lösen.
  ...
  \section{Ablauf\label{sec:Ablauf}}
    ...
    \subsection{Initiale Population\label{sec:InitialPolulation}}
    ...
    \subsection{Parcours-Generierung\label{sec:Parcour Generierung}}
      ...
      % TODO Gossar: asynchronous message-passing
    ...
    \subsection{Selektion\label{sec:Selektion}}
      ...
      \citet[S.33]{book:bioInspired} schreiben, dass Turnierbasierte-Selektion oft bei evolutinärer Programmierung eingesetzt wird.
    ...
    \subsection{Domänenmodel\label{sub:domMod}}
      ...
      Abbildung~\ref{fig:mainProcess} beschreibt den Hauptprozess
      und Abbildung~\ref{fig:simulationProcess} den Simulationsprozess.
