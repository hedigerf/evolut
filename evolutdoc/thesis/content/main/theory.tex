...
\chapter{Grundlagen}
  ...
  \section{Artifizielle Evolution}
    ...
    \subsection{Selektionsoperation}
      ...
      Eine Selektionsoperation hilft geeignetsten Individuen einer Generation zu selektieren.
  ...
  \section{Frozen Accident}
    ...
    Der ``frozen accident'' ist erstmals in einem Artikel von \citet{Crick1968} beschrieben worden.
    \\
    Darin wird beschrieben, dass der genetische Code ein Unfall (``accident'') ist.
    % TODO ergänzen
    \\
    Der genetische Code ist nun eingefroren (``frozen''), da sehr viele Teile angepasst werden müssten
    \\
    \\
    Dies lässt sich auf die künstlichen Individuen übertragen. Am Anfang der Simulation ist die Diversität hoch.
    Es existieren Individuen mit verschiedener Anzahl an Körperpunkten.
    Angenommen ein Individuum mit \( n \) Körperpunkten besitzt die grösste Fitness,
    so wird dieses in der Selektionsphase (\vref{sec:Selektion}) mit einer grösseren Wahrscheinlichkeit ausgewählt,
    als Individuen mit einer anderen Anzahl an Körperpunkten.
    % Weil die Anzahl der Körperpunkte nicht mutiert wird, bleibt 
    \\
